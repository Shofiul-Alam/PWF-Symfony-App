...
//            return $helpers->json($this->accessError);
    ...
    public function updateInductionDataAction(Request $request)
    {
        ...
        $params = json_decode($json);
        $token = $request->get('authorisation', null);
        $inductionId = $this->getEnitityId($params->induction->encryptedId);
        ...
        if ($this->isAuthenticated($token) && $this->isAdmin($token)) {
            ...
            if ($create) {
                ...
                foreach ($params->induction->form->fieldsArr as $field) {
                    $fieldId = $this->getEnitityId($field->fieldId);
                    $userInductionData = $em->getRepository('BackendBundle:UserInductionData')->findOneBy(array('user'=>$user_id, 'induction' => $inductionId, 'fieldId' => $fieldId));
                    if ($field->valueArr != null) {
                        $valueArrId = $this->getEnitityId($field->valueArr->id);
                        $userInductionData->setValueArrId($valueArrId);
                        $em->persist($userInductionData);
...
//                    $field->emptyValueArr();
                    ...
                    $data->getInduction()->getForm()->addFieldsArr($field);
                ...
                if($createNewForm || count($allData) == ($key+1)) {
                    ...
                    $newForm->initFieldsArr($fieldArr);
