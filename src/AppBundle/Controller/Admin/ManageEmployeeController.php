...
class ManageEmployeeController extends AAdmin {
    ...
    public function newAction(Request $request)
    {
        ...
        $avatar = urldecode($request->get('upload', null));
    ...
    public function employeeListAction(Request $request) {
        ...
        if($this->isAuthenticated($token) && $this->isAdmin($token)) {
            ...
            $employees = $em->getRepository("BackendBundle:Employee")->findAll();

            $empList = array();
            foreach ($employees as $employee) {
                $empView = new EmployeeView($employee);
                array_push($empList, $empView);
            }
    ...
    public function getEmployeeByIdAction(Request $request) {
        $helpers = $this->get(Helpers::class);
        ...
        if($this->isAuthenticated($token) && $this->isAdmin($token)) {
            ...
            $em = $this->getDoctrine()->getManager();
            $employee = $em->getRepository("BackendBundle:Employee")->find($this->getEnitityId($id));
            $data = array(
                'status' => "success",
                'code' => 200,
                'msg' => 'Employee details!!!',
    ...
    public function archiveEmployeeListAction(Request $request)
    {
        ...
        if($this->isAuthenticated($token) && $this->isAdmin($token)) {
            ...
            $em = $this->getDoctrine()->getManager();
            $dql = "SELECT c FROM BackendBundle:Employee c WHERE c.archived = 1 ORDER BY c.id DESC";
    ...
    public function registerEmployee(Request $request)
    {
        ...
        $json = urldecode($request->get('json', null));
        $params = json_decode($json);
        $userDeclearation = urldecode($request->get('userDeclearation', null));
    ...
    public function sortEmployeeAction(Request $request) {
        $helpers = $this->get(Helpers::class);
        $json = urldecode($request->get('json', null));
        $params = json_decode($json);
        $token = $request->get('authorisation', null);
        ...
        if($this->isAuthenticated($token) && $this->isAdmin($token)) {
            ...
            $em = $this->getDoctrine()->getManager();
            $query = $em->getRepository('BackendBundle:Employee')->sortEmployeeByUserProp($queryBuilder, $key, $orderBy);
...
//            $data = $this->getModifiedData($pagination->getItems());
    ...
    public function filterEmployeeAction(Request $request) {
        $helpers = $this->get(Helpers::class);
        $json = urldecode($request->get('json', null));
        $params = json_decode($json);
        $token = $request->get('authorisation', null);
        ...
        if($this->isAuthenticated($token) && $this->isAdmin($token)) {
            ...
            $employeeFilter = new EmployeeFilter($filter);

            $em = $this->getDoctrine()->getManager();
            $queryBuilder = $em->createQueryBuilder();

            $em = $this->getDoctrine()->getManager();
            $query = $em->getRepository('BackendBundle:Employee')->filterEmployee($queryBuilder, $employeeFilter);
