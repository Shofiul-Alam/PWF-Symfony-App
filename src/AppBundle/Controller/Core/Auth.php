...
//            $entity->$setMethod(null);
            ...
            $childObj = $em->getRepository('BackendBundle:'.$childEntityName)->find($id);
            if($childEntityName == 'User') {
                $childArr = $childObj->toArray();


                foreach ($childArr as $key=>$childProp) {
                    if(!$this->isEntity($childProp, $childEntityName)) {
                        if(isset($childEntity->{$key}) && $childEntity->{$key} != $childProp && $key != 'id') {
                            $childObj->{'set'.$key}($childEntity->{$key});
                        }
                    }
                }
            }
