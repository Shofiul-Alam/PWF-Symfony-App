...
class AEmployeeController extends Auth {
...
//                $data = $this->getSingleModifiedData($entity);
    ...
    {
        $helpers = $this->get(Helpers::class);
        $token = urldecode($request->get('authorisation', null));
...
//            }
    ...
    protected function addDocument($entity, $upload) {
        $fs = new Filesystem();
        $newName = time(). '_' .$upload->name;

        $fs->copy('tmp/'.$upload->name, 'documents/'.$newName);

        $employeeSkilldocument = new EmployeeSkillDocument();

        $employeeSkilldocument->setFileName($newName);
    ...
    protected function addTimeSheet($timesheet, $upload) {
        $fs = new Filesystem();

        $fs->copy('tmp/'.$upload->name, 'documents/'.$upload->name);

        $timeSheetDoc = new EmployeeTimesheetDocument();

        $timeSheetDoc->setFileName($upload->name);
        $timeSheetDoc->setStorageType($upload->ext);
        $timeSheetDoc->setMime($upload->mime);
        $timeSheetDoc->setSize($upload->size);
        $timeSheetDoc->setTimeSheet($timesheet);
    ...
    protected function updateTimeSheetDocment($documentEntity, $upload) {
        $fs = new Filesystem();
